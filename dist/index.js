...
const parseLevel = (flag) => (value) => {
    const v = value.toLowerCase();
    if (v === "low" || v === "medium" || v === "high")
        return v;
    throw new InvalidArgumentError(`${flag} must be one of: low, medium, high`);
};
const getApiKey = (cliKey) => {
    if (cliKey)
        return cliKey;
    if (process.env.OAI_SEARCH_API_KEY)
        return process.env.OAI_SEARCH_API_KEY;
    if (process.env.OPENAI_API_KEY)
        return process.env.OPENAI_API_KEY;
    console.error("Error: OpenAI API key is required. Pass with --openai-api-key or set OAI_SEARCH_API_KEY / OPENAI_API_KEY.");
    process.exit(1);
};
/* ---------- CLI ---------- */
...
program
    ...
    .option("-r, --reasoning-effort <level>", "reasoning effort level", parseLevel("--reasoning-effort"), "medium")
    .option("-c, --search-context-size <level>", "search context size", parseLevel("--search-context-size"), "medium")
...
const main = async () => {
    ...
    try {
        ...
        const response = await client.responses.create({
            ...
            input: query,
            instructions: "You are a helpful assistant specialized in web search. Your primary role is to search the web for the most relevant and up-to-date information based on the user's query. Provide accurate, comprehensive, and well-structured answers based on the search results. Always cite your sources when presenting information from the web.",
            reasoning: {
